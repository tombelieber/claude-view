...
| **成本追踪** | 每个会话和总体支出——悬停查看 token/成本分解及各类别缓存节省 |
| **子代理可视化** | 看到完整的代理树——子代理、它们的状态，以及它们调用的工具 |
| **多个视图** | 网格、列表、Kanban 或监控模式——选择最适合你工作流的 |
| **Kanban 泳道** | 按项目或分支分组会话——多项目工作流的可视化泳道布局 |
...
| **批量选择与归档** | 选择多个会话进行批量归档，保持筛选状态 |
| **加密分享** | 通过端对端加密链接分享任何会话——零服务器信任 |
...
| **钩子事件记录** | 双通道钩子捕捉（实时 + JSONL 回填）——每个钩子事件都被记录和可浏览，即使是过去的会话 |
| **工作树分支漂移** | 检测 git 工作树分支何时分歧——在实时监控和历史中显示 |
...
| **实时** | 文件监视器 + SSE + 统一 WebSocket，具备心跳、事件重播和崩溃恢复 |
| **足迹很小** | ~10 MB 下载，~27 MB 磁盘空间。无运行时依赖、无后台守护进程 |
| **100% 本地** | 所有数据保留在你的机器上。零遥测、零必要账户。可选加密分享功能可用。 |
...
| `CLAUDE_VIEW_DATA_DIR` | `~/Library/Caches/claude-view` | 覆盖数据目录 |

---

**唯一需求：**[Claude Code](https://docs.anthropic.com/en/docs/claude-code) 已安装——这创建了我们监控的会话文件。
...
- **[claudeview.ai](https://claudeview.ai)** — 官方网站、文档与更新日志
- **[@claude-view/plugin](https://www.npmjs.com/package/@claude-view/plugin)** — Claude Code 插件，提供 8 个 MCP 工具和 3 个技能。`claude plugin add @claude-view/plugin`
- **[claude-backup](https://github.com/tombelieber/claude-backup)** — Claude Code 会在 30 天后删除你的会话。这个工具可以保存它们。`npx claude-backup`
