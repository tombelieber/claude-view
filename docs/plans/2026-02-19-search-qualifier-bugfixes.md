...
|---|-----------|-----|------------|
| 1 | `project:vibe-mom-test` | Returns 0 | Index stores encoded path (`-Users-TBGor-vibe-mom-test`) instead of display name |
...
fn test_search_project_qualifier_with_display_name() {
    ...
    // project:vibe-mom-test should match with display name
...
curl -s 'http://localhost:47892/api/search?q=project%3Avibe-mom-test' | python3 -c "import json,sys; d=json.load(sys.stdin); print(f'project:vibe-mom-test → {d[\"totalSessions\"]} sessions')"
# Expected: > 0 sessions
