...
- **Node**: Full recursive filesystem verification with:
  - `--` → `/@` conversion (scoped packages like `@myorg`)
...
### Impact
- Scoped packages like `@myorg/claude-view` show wrong project name
...
fn test_resolve_scoped_package_path() {
    // Create test directory: /tmp/test/@myorg/claude-view
    let temp = create_temp_scoped_package();

    let resolved = resolve_project_path("-tmp-test--myorg-claude-view");

    assert_eq!(resolved.full_path, "/tmp/test/@myorg/claude-view");
...
fn test_resolve_double_dash_to_at_symbol() {
    ...
    assert!(variants.iter().any(|v| v.contains("/@myorg/")));
}
