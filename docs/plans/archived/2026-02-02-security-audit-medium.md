...
**Files:** `.playwright-mcp/project-a-session-detail.png`, `.playwright-mcp/project-a-sessions-test.png`, `.playwright-mcp/session-view-test.png`

**Problem:** These 3 PNG files were committed before the `.gitignore` rule was added. They expose:
- macOS username (`user`)
- Notion page IDs and URLs
- Internal project names (`@myorg/project-a`, `Famatch.io`, `charity-app`)
