...
export function XmlCard({ content, type }: XmlCardProps) {
  ...
  if (type === 'untrusted_data') {
    ...
    // Security: DOMPurify strips all tags/attributes, React renders as text nodes
    const sanitized =
...
export function detectXmlType(content: string): XmlCardProps['type'] | null {
  if (content.includes('<observed_from_primary_session>')) return 'observed_from_primary_session'
  if (content.includes('<observation>')) return 'observation'
  ...
  if (content.includes('<task-notification>')) return 'task_notification'
  if (content.includes('<command-name>')) return 'command'
  if (content.includes('<tool_use_error>')) return 'tool_error'
  if (/<untrusted-data-[a-f0-9-]+>/.test(content)) return 'untrusted_data'
  if (
    ...
    return 'hidden'
  if (/<[a-z_]+>[\s\S]*<\/[a-z_]+>/i.test(content) && content.length > 100) return 'unknown'
  return null
}
...
export function extractXmlBlocks(content: string): { xml: string; type: XmlCardProps['type'] }[] {
  ...
  while ((gm = genericRegex.exec(content)) !== null) {
    const isOverlapping = matchedRanges.some(
      (range) => gm!.index >= range.start && gm!.index < range.end,
    )
    if (!isOverlapping && gm[0].length > 20) {
      blocks.push({ xml: gm[0], type: 'unknown' })
      matchedRanges.push({ start: gm.index, end: gm.index + gm[0].length })
    }
  }
