...
export const projectsGeneratedTools: ToolDef[] = [
  {
    ...
    description: 'list all projects backed by the in-memory catalog.',
  ...
  },
  {
    ...
    handler: async (client, args) => {
      const result = await client.request(
        'GET',
        `/api/projects/${encodeURIComponent(String(args.id))}/branches`,
      )
      return JSON.stringify(result, null, 2)
    },
  },
  {
    ...
    description: 'paginated sessions for one project.',
    inputSchema: z.object({
      id: z.string(),
    ...
    handler: async (client, args) => {
      const result = await client.request(
        'GET',
        `/api/projects/${encodeURIComponent(String(args.id))}/sessions`,
        {
          params: {
            limit: args.limit,
            offset: args.offset,
