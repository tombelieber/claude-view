...
export const settingsGeneratedTools: ToolDef[] = [
  {
    name: 'settings_get_claude_code_settings',
    description: 'returns merged, redacted Claude Code settings.',
    inputSchema: z.object({}),
    annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    handler: async (client, _args) => {
      const result = await client.request('GET', '/api/claude-code-settings')
  ...
  },
  {
    ...
    handler: async (client, args) => {
      const result = await client.request('PUT', '/api/settings', {
        body: { llmModel: args.llmModel, llmTimeoutSecs: args.llmTimeoutSecs },
      })
      return JSON.stringify(result, null, 2)
    },
  },
  {
    ...
    annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
    handler: async (client, args) => {
      const result = await client.request('PUT', '/api/settings/git-sync-interval', {
        body: { intervalSecs: args.intervalSecs },
      })
      return JSON.stringify(result, null, 2)
    },
  },
