...
export const pluginsGeneratedTools: ToolDef[] = [
  ...
  {
    name: 'plugins_marketplace_action',
    description: 'Marketplace Action (POST /api/plugins/marketplaces/action)',
    inputSchema: z.object({
      action: z.string().describe('"add" | "remove" | "update"'),
      name: z.string().describe('For remove/update: marketplace name').optional(),
      scope: z.string().describe('For add: "user" | "project"').optional(),
      source: z.string().describe('For add: GitHub repo URL or owner/repo').optional(),
    }),
    annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
    handler: async (client, args) => {
      const result = await client.request('POST', '/api/plugins/marketplaces/action', {
        body: { action: args.action, name: args.name, scope: args.scope, source: args.source },
      })
      return JSON.stringify(result, null, 2)
    },
  },
