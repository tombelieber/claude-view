...
| **成本追蹤** | 每個會話和總體支出——懸停查看 token/成本分解及各類別快取節省 |
| **子代理可視化** | 看到完整的代理樹——子代理、它們的狀態，以及它們調用的工具 |
| **多個檢視** | 網格、列表、Kanban 或監控模式——選擇最適合你工作流的 |
| **Kanban 泳道** | 按專案或分支分組會話——多專案工作流的視覺化泳道佈局 |
...
| **批次選擇與封存** | 選擇多個會話進行批次封存，保持篩選狀態 |
| **加密分享** | 透過端對端加密連結分享任何會話——零伺服器信任 |
...
| **鉤子事件記錄** | 雙通道鉤子捕捉（即時 + JSONL 回填）——每個鉤子事件都被記錄和可瀏覽，即使是過去的會話 |
| **工作樹分支漂移** | 偵測 git 工作樹分支何時分歧——在即時監控和歷史中顯示 |
...
| **即時** | 檔案監視器 + SSE + 統一 WebSocket，具備心跳、事件重播和崩潰恢復 |
| **足跡很小** | ~10 MB 下載，~27 MB 磁碟空間。無執行時依賴、無背景守護進程 |
| **100% 本地** | 所有資料保留在你的機器上。零遙測、零必要帳戶。可選加密分享功能可用。 |
...
| 方式 | 命令 |
|--------|---------|
| **Shell**（推薦） | `curl -fsSL https://get.claudeview.ai/install.sh \| sh` |
| **npx** | `npx claude-view` |
