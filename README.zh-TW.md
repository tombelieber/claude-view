...
claude-view 是一個開源儀表板，監控你機器上每一個 Claude Code session — 即時 agent、歷史對話、費用、sub-agent、hook、工具呼叫 — 全部集中在一個地方。Rust 後端、React 前端、約 10 MB 的執行檔。零設定、零帳號、本地優先 —— 你的會話內容不會離開你的機器（官方構建會發送匿名功能使用分析，可用 `CLAUDE_VIEW_TELEMETRY=0` 退出；源碼構建不發送任何資料）。
...
| **本地優先** | 你的會話、代碼和提示詞從不離開你的機器。官方構建（`npx claude-view` / install.sh）會發送匿名功能使用分析（絕不含內容）—— 用 `CLAUDE_VIEW_TELEMETRY=0` 退出；源碼構建什麼都不發送。零必要帳號 |
...
|------|----------|-------|:----:|:------------:|:------------------:|:------:|:---------:|:---------:|
| **[claude-view](https://github.com/tombelieber/claude-view)** | 監控 + 工作區 | Rust | **~10 MB** | **是** | **是** | **是** | **是** | **85** |
...
claude-view 讀取 Claude Code 寫入 `~/.claude/projects/` 的 JSONL session 檔案，使用 SQLite 在本地建立索引。**你的會話內容不會離開你的機器**，除非你主動使用加密分享功能。官方構建（`npx claude-view` / install.sh）會發送**匿名使用分析** —— 哪些功能/頁面被打開、app 版本、作業系統、安裝來源、隨機的安裝級 UUID —— 但**絕不**包含代碼、提示詞、路徑或會話內容；隨時用 `CLAUDE_VIEW_TELEMETRY=0` 退出。源碼構建不含分析密鑰，什麼都不發送。詳見 [Privacy & Telemetry](./README.md#privacy--telemetry)。
