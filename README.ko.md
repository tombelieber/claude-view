...
| **비용 추적** | 세션별 및 전체 지출 — 호버하여 토큰/비용 분석과 카테고리별 캐시 절약 내역 확인 |
| **서브 에이전트 시각화** | 전체 에이전트 트리 — 서브 에이전트, 상태, 호출 중인 도구 확인 |
| **다중 뷰** | 그리드, 리스트, Kanban 또는 모니터 모드 — 워크플로우에 맞게 선택 |
| **Kanban 스윔레인** | 프로젝트 또는 브랜치별로 세션 그룹화 — 멀티 프로젝트 워크플로우를 위한 시각적 스윔레인 레이아웃 |
...
| **일괄 선택 및 보관** | 여러 세션을 선택하여 필터 상태를 유지하며 일괄 보관 |
| **암호화 공유** | E2E 암호화 링크로 모든 세션 공유 — 서버 신뢰 제로 |
...
| **훅 이벤트 기록** | 듀얼 채널 훅 캡처(라이브 + JSONL 백필) — 과거 세션을 포함한 모든 훅 이벤트가 기록되고 탐색 가능 |
| **워크트리 브랜치 드리프트** | git 워크트리 브랜치가 분기될 때 감지 — 라이브 모니터와 히스토리에 표시 |
...
- **워크트리 인식** — git 워크트리 간 브랜치 드리프트 감지
...
| **실시간** | 파일 워처 + SSE + 통합 WebSocket(하트비트, 이벤트 리플레이, 크래시 복구 지원) |
| **작은 풋프린트** | ~10 MB 다운로드, ~27 MB 디스크 사용. 런타임 의존성 없음, 백그라운드 데몬 없음 |
| **100% 로컬** | 모든 데이터는 여러분의 머신에. 텔레메트리 제로, 필수 계정 없음. 선택적 암호화 공유 기능 제공. |
...
**유일한 요구사항:** [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 설치됨 — 모니터링할 세션 파일을 생성합니다.
