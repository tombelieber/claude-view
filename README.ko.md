...
> ⚠️ **이 번역은 오래되었을 수 있습니다 (개인정보 / 텔레메트리)** — 공식 빌드는 이제 기본적으로 익명 사용 분석을 전송합니다. 최신의 정식 세부 정보는 [Privacy & Telemetry (English README)](./README.md#privacy--telemetry)를 참조하세요. `CLAUDE_VIEW_TELEMETRY=0`으로 옵트아웃하세요.


## claude-view란?

claude-view는 머신에서 실행되는 모든 Claude Code 세션을 모니터링하는 오픈소스 대시보드입니다 — 라이브 에이전트, 과거 대화, 비용, 서브에이전트, 훅, 도구 호출을 한곳에서 확인하세요. Rust 백엔드, React 프론트엔드, ~10 MB 바이너리. 설정 불필요, 계정 불필요, 로컬 우선 — 세션 콘텐츠가 머신을 벗어나지 않습니다 (공식 빌드는 익명 사용 분석을 전송; 옵트아웃: `CLAUDE_VIEW_TELEMETRY=0`; 소스 빌드는 아무것도 전송하지 않음).
...
| **Local-first** | Your session content stays on your machine. Official builds send anonymous feature-usage analytics (no content, ever) — opt out with `CLAUDE_VIEW_TELEMETRY=0`. Source builds send nothing. 정식 세부 정보: [Privacy & Telemetry (EN)](./README.md#privacy--telemetry) |
...
claude-view는 Claude Code가 `~/.claude/projects/`에 기록하는 JSONL 세션 파일을 읽습니다. SQLite와 Tantivy를 사용하여 로컬에서 인덱싱합니다. **Your session content never leaves your machine** unless you explicitly use the encrypted Share feature. Official builds send anonymous usage analytics (feature counts only, never content) — opt out with `CLAUDE_VIEW_TELEMETRY=0`. Source builds send nothing. 정식 세부 정보: [Privacy & Telemetry (EN)](./README.md#privacy--telemetry).
