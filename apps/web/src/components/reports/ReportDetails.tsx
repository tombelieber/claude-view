    ...
    <div className="mt-4 border-t border-gray-100 dark:border-gray-800 pt-3">
      ...
      >
        ...
        {!expanded && props.generationModel && (
          <span className="ml-1 text-gray-300 dark:text-gray-600">
            · Generated by{' '}
            <span title={props.generationModel}>{formatModelName(props.generationModel)}</span>
          </span>
        )}
      </button>
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-1 text-gray-600 dark:text-gray-400">
                <span>
                  ...
                  <span
                    ...
                    title={props.generationModel}
                  >
                    {formatModelName(props.generationModel)}
                  </span>
                </span>
                {props.generationInputTokens != null && props.generationInputTokens > 0 && (
                  <span>
                    (
                    {formatTokens(
                      props.generationInputTokens + (props.generationOutputTokens ?? 0),
                    )}{' '}
                    tokens)
                  </span>
                )}
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-1 text-gray-600 dark:text-gray-400">
              ...
              {digest.total_input_tokens != null && digest.total_input_tokens > 0 && (
                <span>
                  Session tokens:{' '}
                  <span className="text-gray-900 dark:text-gray-200 font-medium">
                    {formatTokens(digest.total_input_tokens)}
                  </span>{' '}
                  in /{' '}
                  <span className="text-gray-900 dark:text-gray-200 font-medium">
                    {formatTokens(digest.total_output_tokens ?? 0)}
                  </span>{' '}
                  out
                </span>
              )}
                  ...
                  <div key={proj.name}>
                    <div className="flex items-baseline gap-1.5">
                      ...
                      <span className="text-gray-500 dark:text-gray-400">
                        {proj.session_count} sessions &middot;{' '}
                        {formatDuration(proj.total_duration_secs)}
