...
describe('CostBreakdown', () => {
  it('renders total cost', () => {
    const cost = { totalUsd: 2.34, inputCostUsd: 1.50, outputCostUsd: 0.84, cacheReadCostUsd: 0.10, cacheCreationCostUsd: 0.05, cacheSavingsUsd: 0.50, isEstimated: false }
    render(<CostBreakdown cost={cost} subAgents={[]} />)
    expect(screen.getByText('$2.34')).toBeInTheDocument()
  })

  it('renders sub-agent costs when present', () => {
    const cost = { totalUsd: 5.00, inputCostUsd: 3.00, outputCostUsd: 2.00, cacheReadCostUsd: 0, cacheCreationCostUsd: 0, cacheSavingsUsd: 0, isEstimated: false }
    const subAgents = [
