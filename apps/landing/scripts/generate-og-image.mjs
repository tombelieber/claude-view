...
for (let y = 0; y < H; y++) {
  ...
  for (let x = 0; x < W; x++) {
    ...
    if (y < 6) {
      raw[p] = 0x22; raw[p+1] = 0xc5; raw[p+2] = 0x5e; // #22c55e green
    } else {
      raw[p] = 0x0f; raw[p+1] = 0x17; raw[p+2] = 0x2a; // #0f172a dark slate
    }
  }
}
...
console.log(`og-image.png created (${W}x${H}, ${png.length} bytes)`);
