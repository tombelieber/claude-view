...
> ⚠️ **翻訳が古い可能性があります（プライバシー / テレメトリー）** — 公式ビルドは現在、匿名の使用統計をデフォルトでオンにしています。最新の正確な情報は [Privacy & Telemetry (English README)](./README.md#privacy--telemetry) を参照してください。`CLAUDE_VIEW_TELEMETRY=0` でオプトアウト。


## claude-view とは？

claude-view は、マシン上のすべての Claude Code セッションを監視するオープンソースダッシュボードです。ライブエージェント、過去の会話、コスト、サブエージェント、フック、ツールコールをすべて一箇所で確認できます。Rust バックエンド、React フロントエンド、約10 MBのバイナリ。設定不要、アカウント不要、ローカルファースト — セッションコンテンツがマシンから出ることはありません（公式ビルドは匿名の使用統計を送信；オプトアウト: `CLAUDE_VIEW_TELEMETRY=0`；ソースビルドは何も送信しません）。

**30リリース。85 MCPツール。9スキル。`npx claude-view` 1つで。**
...
`@claude-view/plugin` は、Claude にダッシュボードデータへのネイティブアクセスを提供します — 85 MCPツール、9スキル、自動起動。
...
| **Local-first** | Your session content stays on your machine. Official builds send anonymous feature-usage analytics (no content, ever) — opt out with `CLAUDE_VIEW_TELEMETRY=0`. Source builds send nothing. 正規の詳細: [Privacy & Telemetry (EN)](./README.md#privacy--telemetry) |
...
|------|----------|-------|:----:|:------------:|:------------------:|:------:|:---------:|:---------:|
| **[claude-view](https://github.com/tombelieber/claude-view)** | モニター + ワークスペース | Rust | **約10 MB** | **対応** | **対応** | **対応** | **対応** | **85** |
...
claude-view は Claude Code が `~/.claude/projects/` に書き込むJSONLセッションファイルを読み取ります。SQLite と Tantivy を使用してローカルにインデックスを作成します。**Your session content never leaves your machine** unless you explicitly use the encrypted Share feature. Official builds send anonymous usage analytics (feature counts only, never content) — opt out with `CLAUDE_VIEW_TELEMETRY=0`. Source builds send nothing. 正規の詳細: [Privacy & Telemetry (EN)](./README.md#privacy--telemetry).
...
- **プラグイン：** [`@claude-view/plugin`](https://www.npmjs.com/package/@claude-view/plugin) — 85 MCPツール、9スキル、自動起動
...
- **[@claude-view/plugin](https://www.npmjs.com/package/@claude-view/plugin)** — 85 MCPツールと9スキルを備えた Claude Code プラグイン。`claude plugin add @claude-view/plugin`
