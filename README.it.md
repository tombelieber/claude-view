...
| Metodo | Comando |
|--------|---------|
| **Shell** (raccomandato) | `curl -fsSL https://raw.githubusercontent.com/tombelieber/claude-view/main/install.sh \| sh` |
| **npx** | `npx claude-view` |

L'installer shell scarica un binario pre-compilato (~10 MB), lo installa in `~/.claude-view/bin` e lo aggiunge al PATH. Poi basta eseguire `claude-view`.
...
| `CLAUDE_VIEW_DATA_DIR` | `~/Library/Caches/claude-view` | Directory dei dati |

---

**Unico requisito:** [Claude Code](https://docs.anthropic.com/en/docs/claude-code) installato — questo crea i file di sessione che monitoriamo.
...
- **[@claude-view/plugin](https://www.npmjs.com/package/@claude-view/plugin)** — Plugin per Claude Code con 8 strumenti MCP e 3 skill. `claude plugin add @claude-view/plugin`
- **[claude-backup](https://github.com/tombelieber/claude-backup)** — Claude Code elimina le tue sessioni dopo 30 giorni. Questo strumento le salva. `npx claude-backup`
