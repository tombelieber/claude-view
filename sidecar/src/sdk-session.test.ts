...
//
// This function was introduced after the same bug (empty sessionId → broken
// navigation → no live session) recurred 6 times. These tests guard against
// every known regression path.
...
describe('waitForSessionInit', () => {
  ...
  // --- Unit: resolves when session_init event fires ---
  it('resolves when session_init event fires and sessionId is populated', async () => {
    const cs = makeStubCs()
    const promise = waitForSessionInit(cs)

    // Simulate: SDK stream emits session_init after a tick
    setTimeout(() => {
  ...
  it('rejects when a fatal error event fires before session_init', async () => {
    const cs = makeStubCs()
    const promise = waitForSessionInit(cs)
  ...
  it('rejects after timeout when session_init never fires', async () => {
    const cs = makeStubCs()
  ...
  // --- Edge: ignores non-fatal error events, waits for session_init ---
  it('ignores non-fatal error events and keeps waiting', async () => {
    ...
    // session_init fires after the non-fatal error
    setTimeout(() => {
      cs.sessionId = 'session-after-warning'
      cs.emitter.emit('message', { type: 'session_init', seq: 1 })
    }, 10)
    vi.advanceTimersByTime(10)
  ...
  it('ignores assistant_text and other unrelated events', async () => {
    ...
    setTimeout(() => {
      ...
      cs.emitter.emit('message', { type: 'session_init', seq: 2 })
    }, 10)
    vi.advanceTimersByTime(10)
  ...
  it('removes emitter listener after resolving', async () => {
    const cs = makeStubCs()
    const listenerCountBefore = cs.emitter.listenerCount('message')

    const promise = waitForSessionInit(cs)
    expect(cs.emitter.listenerCount('message')).toBe(listenerCountBefore + 1)

    cs.sessionId = 'sid'
    cs.emitter.emit('message', { type: 'session_init', seq: 0 })
    await promise

    expect(cs.emitter.listenerCount('message')).toBe(listenerCountBefore)
  })

  // --- Regression: listener cleanup after reject (fatal error) ---
  it('removes emitter listener after rejecting on fatal error', async () => {
    const cs = makeStubCs()
    const listenerCountBefore = cs.emitter.listenerCount('message')
  ...
  it('removes emitter listener after timeout', async () => {
    const cs = makeStubCs()
    const listenerCountBefore = cs.emitter.listenerCount('message')
  ...
  it('waits for session_init when sessionId is empty string', async () => {
  ...
  it('resolves when session_init fires synchronously after listen', async () => {
    ...
    // Pre-register an immediate emitter that fires session_init
