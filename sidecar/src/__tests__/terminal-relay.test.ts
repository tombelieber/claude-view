...
describe('parseClientMessage', () => {
  ...
  it('returns null for resize with negative rows', () => {
    expect(
      parseClientMessage('{"type":"resize","cols":80,"rows":-1}'),
    ).toBeNull()
  })

  it('returns null for resize with excessively large cols', () => {
    expect(
      parseClientMessage('{"type":"resize","cols":501,"rows":24}'),
    ).toBeNull()
  })

  it('returns null for resize with excessively large rows', () => {
    expect(
      parseClientMessage('{"type":"resize","cols":80,"rows":201}'),
    ).toBeNull()
  })

  it('returns null for resize with non-numeric cols', () => {
    expect(
      parseClientMessage('{"type":"resize","cols":"abc","rows":24}'),
    ).toBeNull()
  })
  ...
  it('accepts boundary values (1x1 and 500x200)', () => {
    ...
    expect(
      parseClientMessage('{"type":"resize","cols":500,"rows":200}'),
    ).toEqual({ type: 'resize', cols: 500, rows: 200 })
  })
})
...
describe('handleTerminalWebSocket', () => {
  ...
  it('forwards binary messages to pty.write', () => {
    ...
    )?.[1] as (data: Buffer | string, isBinary: boolean) => void
  ...
  it('cleans up session when last client disconnects', () => {
    ...
    // Simulate close
    const onCalls = (ws.on as ReturnType<typeof vi.fn>).mock.calls
    const closeHandler = onCalls.find(
