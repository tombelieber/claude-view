//! Merge-strategy wrappers for typed field semantics.
//!
//! `Monotonic<T>`, `Latest<T>`, and `Transient<T>` enforce merge rules at
//! compile time. Originally defined in `mutation::merge`; canonical home is
//! now here so the state crate is self-contained.
