...
async fn test_list_project_summaries() {
    ...
    assert!(
        ...
        "Summaries should NOT include sessions array"
    );
    assert!(
        json.contains("\"sessionCount\""),
        "Should have sessionCount field"
    );
}
...
async fn test_list_sessions_for_project_pagination() {
    ...
    let page1 = db
        .list_sessions_for_project("project-a", 2, 0, "recent", &BranchFilter::All, false)
        .await
        .unwrap();
    ...
    let page2 = db
        .list_sessions_for_project("project-a", 2, 2, "recent", &BranchFilter::All, false)
        .await
        .unwrap();
...
async fn test_list_sessions_for_project_sort() {
    ...
    let s2 = SessionInfo {
        message_count: 5,
        ..make_session("sess-2", "project-a", 3000)
    };
    let s3 = SessionInfo {
        message_count: 50,
        ..make_session("sess-3", "project-a", 2000)
    };
    ...
    // Sort by oldest
    let oldest = db
        .list_sessions_for_project("project-a", 10, 0, "oldest", &BranchFilter::All, false)
        .await
        .unwrap();
    ...
    // Sort by messages
    let by_msg = db
        .list_sessions_for_project("project-a", 10, 0, "messages", &BranchFilter::All, false)
        .await
        .unwrap();
...
async fn test_list_sessions_excludes_sidechains() {
    ...
    let page = db
        .list_sessions_for_project("project-a", 10, 0, "recent", &BranchFilter::All, false)
        .await
        .unwrap();
    ...
    // Include sidechains
    let page = db
        .list_sessions_for_project("project-a", 10, 0, "recent", &BranchFilter::All, true)
        .await
        .unwrap();
...
async fn test_project_summaries_exclude_sidechains() {
    ...
    assert_eq!(
        summaries[0].session_count, 1,
        "Sidechain should be excluded from count"
    );
}
...
async fn test_list_sessions_for_project_includes_phase3_fields() {
    ...
    db.update_session_deep_fields(
        ...
        15,
        20,
        30, // user_prompt_count, api_call_count, tool_call_count
        r#"["/a.rs"]"#,
        r#"["/b.rs"]"#, // files_read, files_edited
        1,
        1,
        0, // counts
        120,
        2,    // duration_seconds, commit_count
    ...
    let page = db
        .list_sessions_for_project("proj-paginated", 10, 0, "recent", &BranchFilter::All, false)
        .await
        .unwrap();
