...
//
// The session_stats full-row writer lives in `upsert_stats.rs` — see
// `execute_upsert_session_stats_from_parsed`. Keeping these concerns in
// sibling files instead of one 600-line monolith follows the
// "Decompose, Don't Monolith" rule.
...
// The session_stats full-row UPSERT (SQL + executor) lives in
// `upsert_stats.rs`. See `execute_upsert_session_stats_from_parsed` and
// `UPSERT_SESSION_STATS_FROM_PARSED_SQL` — both re-exported from
// `queries::sessions` for the Phase 7.h writers.
