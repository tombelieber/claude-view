...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !claude_dir.exists() {
        eprintln!(
            "Error: {} does not exist. This benchmark requires a real ~/.claude directory.",
            claude_dir.display()
        );
        std::process::exit(1);
    }
    ...
    println!(
        "Build hints: {} sessions in {:.1}ms",
        hints.len(),
        hints_elapsed.as_secs_f64() * 1000.0
    );
    ...
    println!(
        "Initial scan: {} indexed, {} skipped in {:.1}ms",
        indexed,
        skipped,
    ...
    println!(
        "Subsequent scan: {} indexed, {} skipped in {:.1}ms",
        indexed2,
        skipped2,
        rescan_elapsed.as_secs_f64() * 1000.0
    );
    ...
    println!(
        "Initial scan: {:>8.1}ms  (target: <1000ms)   {}",
        scan_ms,
        if scan_ms < 1000.0 { "PASS" } else { "MISS" }
    );
    println!(
        "Rescan:       {:>8.1}ms  (target: <10ms)    {}",
        rescan_ms,
        if rescan_ms < 10.0 { "PASS" } else { "MISS" }
    );
